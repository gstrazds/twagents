...
def get_failures(stats):
    ...
    for gamefile in stats:
        for no_episode in range(len(stats[gamefile]["runs"])):
            ...
            max_score = stats[gamefile]["max_scores"]
            if score < max_score:
                print(f"[{no_episode}] {score}/{max_score}  {gamefile}")
                fails += 1
    return fails
...
def score_leaderboard(stats, output_dir):
    print("fails =", get_failures(stats["games"]))
...
def main():
    ...
    try:
        ...
        msg = ("Cannot find a valid output for the ingestion program. Most certainly it has failed."
               ...
               "\n\n-> the `metadata` file is missing. Make sure you include a file"
               " called `metadata` in your submission zip file (see sample_submission_lstm-dqn.zip)."
               "\n\n-> multiple instances of your agent are being run in parallel."
