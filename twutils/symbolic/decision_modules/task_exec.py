...
class TaskExecutor(DecisionModule):
    """
    The TaskExecutor module activates sequences and executes Tasks.
    """
    ...
    def take_control(self, gi: GameInstance):
        ...
        while self.recipe_steps:
            ...
            if step:
                ...
                print("TaskExec FAILED to convert next step to action", self.recipe_steps)
                success = False
            if step and success:
                # self.recipe_steps = self.recipe_steps[1:]
                print("TaskExec recipe_steps <= ", self.recipe_steps)
            else:
                print("TaskExec stopping: failed to execute next step")
                self.deactivate()
                return None
    ...
    #     self.record(success)
    #     #gv.dbg(
    #     print("[TaskExec](1.success={}) {} --> {}".format(
    #         success, PrepareMeal, response))
        if not success:
            print("TaskExec FAILED TO PREPARE MEAL")
            gi.event_stream.push(NeedToGoTo('kitchen', groundtruth=True))
        ...
        self.record(success)
        #gv.dbg(
        print("[TaskExec](2.success={}) {} --> {}".format(
